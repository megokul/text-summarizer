    ...
    def _save_model_and_tokenizer(self, model, tokenizer):
        ...
        Returns:
            (
                ...
                final_tokenizer_dir: Path | None,
                model_s3_uri: str | None,
                tokenizer_s3_uri: str | None,
                final_model_s3_uri: str | None,
                final_tokenizer_s3_uri: str | None,
            )
        """
        # --- Local Paths ---
        model_dir = self.trainer_config.model_dir      # e.g. Path to "pegasus-samsum-model"
        ...
        if self.trainer_config.s3_enabled and self.backup_handler is not None:
            with self.backup_handler as handler:
                # Main model dir to S3
                if model_dir and model_s3_key:
                    model_s3_uri = handler.upload_dir(model_dir, model_s3_key)
                    logger.info(f"Model directory uploaded to S3: {model_s3_uri}")
                if tokenizer_dir and tokenizer_s3_key:
                    tokenizer_s3_uri = handler.upload_dir(tokenizer_dir, tokenizer_s3_key)
                    logger.info(f"Tokenizer directory uploaded to S3: {tokenizer_s3_uri}")

                # Final model dir to S3
                if final_model_dir and final_model_s3_key:
                    final_model_s3_uri = handler.upload_dir(final_model_dir, final_model_s3_key)
                    logger.info(f"Final model directory uploaded to S3: {final_model_s3_uri}")
                if final_tokenizer_dir and final_tokenizer_s3_key:
                    final_tokenizer_s3_uri = handler.upload_dir(final_tokenizer_dir, final_tokenizer_s3_key)
